...
class pulsr:
    ...
    def receive_data(self):
        """
        Receives a line of data from the Arduino, decodes it, and returns as a string.
        Returns None if no data is available.
        """
        if self.pulsr2_coms.in_waiting > 0:
            try:
                data = self.pulsr2_coms.readline().decode('utf-8', errors='ignore').strip()
                print(f"Received: {data}")  # Optional: for debugging
                return data
